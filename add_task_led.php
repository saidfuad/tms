...
if (isset($_POST['cancel'])) {
	...
	$SQL = "UPDATE info SET individ_task = '$atask' WHERE username = '$unem'";
	mysql_query($SQL);
	
	//print("<SCRIPT LANGUAGE='JavaScript'>alert('Checking! $atask')</script>");
	
	$SQL = "INSERT INTO $table_task (`username`, $atask) VALUES ('$unem', 'pending')";
	mysql_query($SQL);
	
	$SQL = "UPDATE info SET task_status_indi = 'pending' WHERE username = '$unem'";
	mysql_query($SQL);
	...
	print("<input name = '' type = 'hidden' value = $tsk>");
	print("</tr>");	
	print("<tr>");
